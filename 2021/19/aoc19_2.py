...
def run():
    ...
    maxDistance = 0
    for s0i, s0 in enumerate(scanners.keys()):
        for s1i, s1 in list(enumerate(scanners.keys()))[s0i + 1:]:
            distance = abs(s0[0] - s1[0]) + \
                abs(s0[1] - s1[1]) + abs(s0[2] - s1[2])
            maxDistance = max(distance, maxDistance)
            # print(s0i, s1i, distance)
            pass
